...
void main()
{
    // Get the model position of the vertex
    vec4 modelPosition = modelMatrix * vec4(position, 1.0);

    // Elevation based on the x component of modelPosition, with the frequency uniform applied
...
{
elevation -= abs(cnoise(vec3(modelPosition.xz * uSmallWavesFrequency
* i, uTime * uSmallWavesSpeed)) * uSmallWavesElevation / i);
}

    // Apply the calculated elevation to the y-axis
    ...
    // Final position to be used for rendering
